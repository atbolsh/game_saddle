...
* **Rerun t1** (seconds — `parse_target` / TARGET line; `oracle_verdict`
  END_GAME correct/wrong; `_oracle_meta` validation + fallback +
  `oracle_target_ok`; `target_missing` drop parity with missing RATING
  (perception excused); `agent_exited` disc contact; `_multi_gold_stop`
  `exited` arg; `DATAGEN_ROOM_FLAG` on `_datagen` and absent from
  `_smoke_eval`). Datagen start heals the new
  `core_analyst_045_target_line` Preference. Exit is disc contact with
  the unit-square boundary, not the agent center leaving `[0,1]`.
  Missing TARGET is dropped like missing RATING; perception rounds are
  the only excuse.
