...
| 1 | `python -m training.selftest t1` | seconds | pure units: NAMS `ExtractorType.NONE` (auto-NER off); `parse_rating` (incl. bold variants and a leading `[ANALYST]` tag), `parse_target` (`TARGET: gold|openings, <0-based index>` / `TARGET: none`, last-wins, `[ANALYST]` prefix, leading `Analyst:` self-label on the same line; bad kind / missing index / mid-prose mention → None), the 2026-08-05 SHAPE/SCALE reward split — `rating_advantage` (exp-advantage vs a baseline: 1.0 at the mean, ~2x per +0.2, hard floor 0 at r ≤ -0.5, cap `ADV_CAP` 3.0) and `example_scale` (ADDITIVE win boost `1.0 * 0.95^d` and eat boost `0.5 * 0.95^d` from `gold_collected`, both rescue a floored reply, exact-0 floor otherwise) on the SCALE side; `build_span_weights` on the SHAPE side (base 1.0, WRONG spans at −0.5 = bounded unlikelihood, oracle move-token modifier LAST and only on correct/wrong verdicts); `oracle_verdict` geometry (15 cases: exact match, either-turn ≥170°, in-cone (20°) FORWARD neutral, 28.6° FORWARD wrong (the 2026-08-13 cone shrink), ray-hit turns WRONG in both directions (the 2026-08-11 missed-forward tightening), out-of-cone/away-turn wrong, unknown on missing meta or perception rounds, `[END_GAME]` vs gold remaining = wrong, `[END_GAME]` vs `oracle_move=END_GAME` = correct, board move on sealed empty = wrong) and `_oracle_meta` validation + fallback + `oracle_target_status` (valid gold pick honored including the farther of two, `"ok"`; a REAL object the rules forbid — an opening or `none` while gold remains — is `"mismatch"`: a player mistake that trains, oracle aimed at nearest gold else nearest opening else END_GAME; a missing line or a nonexistent object — OOB index, absent kind — is `"corrupt"`: dropped train-side; `TARGET: none` on sealed empty is `END_GAME` `"ok"`; missing on sealed empty is `END_GAME` `"corrupt"`); `agent_exited` disc-contact (interior False, per-side True, radius toward the tested edge); `action_balance_multipliers` (TEMPORARY hack: inverse-frequency mean/count per action, mass-preserving, cap 4x both ways on degenerate mixes), `NoveltyTracker` (WIP boredom decay: 0.9^k, floor 0.1, reset on a different move, perception rounds skipped WITHOUT resetting, per-game keys) standalone AND through `GameTraceSource` with `novelty=True` (OFF by default — a default source must NOT decay); `GameTraceSource` integration on fabricated corpora: span weights stay pure shape while `example_weight` carries exp-advantage × balance × novelty × oracle penalty × ray-hit `TRANSITION_BOOST` (r_bar pre-pass asserted, oracle-wrong record gets ×0.25 + a −0.5 move span, missed-forward record gets ×0.25 × 2.0 + the −0.5 span, ray-hit FORWARD gets ×2.0, floored records SKIPPED, rating-null DROPPED, `target_missing`/`target_invalid` DROPPED like missing RATING (perception-excused KEPT, old corpora without the keys KEPT)), `MetricGuard` ceiling-only mode (`relative=False`) and `warn_only`, image-noise determinism/identity + the 10% clean pass-through (`_SKIP_PROB`), analyst-leak tripwire (incl. multi-line), `_rewrite_image_urls`, `PlayerAnchorSource` (kd_anchor loss, rating-null KEPT, uniform weights, 0.25 weight, ceiling-only guard at 1.0), micro-batch bucketing, planted-error scrambler, perception-question sampling, `AnalystTraceSource` (KD loss kind, rating-null kept, unverified-span record dropped, quota weight, ceiling-only guard at 5.0), `stack_equal_length`; `partition_prefill_batches` (T>=8700 longs cap at 3, shorts stay one batch); `run_weekend --checkpoint` (rejects `--start-checkpoint` / `--resume-checkpoint`); datagen `--multi-gold` vs smoke sealed (`DATAGEN_ROOM_FLAG`); `_multi_gold_stop` (eat-gold is not a terminal; `[END_GAME]` / disc-contact walk-out win kinds; `exited` arg); 2026-08-12: `boundary_openings` (full walls → empty, fabricated right-wall gap, no walls → 4 sides, rotated boundary wall → ValueError), `new_multi_gold_game` (n_gold=0/2/3, opening require/forbid), `[END_GAME]` parser on the multi-gold subclass AND the base class; universal `openings` on every `settings_to_dict`; `oracle_verdict` END_GAME correct/wrong; `action_balance_multipliers` pins END_GAME at 1.0; unified prompt composition (`PICK ONE TARGET AND COMMIT` present, multi-move wording gone, debrief uses `RATING:` not `overall_score`); 2026-08-14: `parse_remember_notes` (order, overwrite-in-order, case-fold, malformed ignored, value-to-`]`, strip), truncation+parse composition, `board_update_line` (eaten / not-eaten), `format_notepad` (empty / populated); 2026-08-26 counted turns: `parse_move` / `find_invalid_move_token` / `PLAYER_STOP_PATTERN` (matches `[CLOCK 12]`/`[ANTICLOCK 60]`/`[CLOCK]`/`[FORWARD]`/`[END_GAME]`, not `[FORWARD 10]`/`[CLOCK 0]`/`[CLOCK 61]`/`[CLOCK 90]`), `swivel_clock_n` vs 7 sequential swivels, `oracle_verdict` `count_off` (bare n=1 correct iff `steps_correct <= 4`; floor-3 and 10% regimes; either-turn ~30 in tolerance; long-way still wrong), `build_span_weights` digit-run split on counted `count_off`, `_game_time_from_end` `[1,10,0,1]→[11,10,0,0]` / all-1s reproduces `last-i`, duration-aware `moves_from_eat` / `GameTraceSource` eat boost at `0.95**10` |
...
| 9 | `python -m training.selftest t9` | ~10–25 min | t8's EXACT workload at `--parallel 3` (run t8 first). Answers “did phase-lock + padded / prefix-KV batch actually engage?” Speedup vs t8 is reported; only asserts no ≥1.6× slowdown. t8+t9 together can exceed 1h and are **not** the weekend estimate (`run_weekend` default `--parallel 12`) |
...
| 11 | `python -m training.selftest t11` | ~25–50 min | Weekend-shaped datagen estimate: **12 games × 4 moves at `--parallel 12`**. Does **not** require t8. Prints warmup + steady + `warmup_s + (3000 − n_warmup) × steady`. 8×4 at p12 leaves four workers idle. |
...
(`RATING: -1..1`, not 0–10). Training stays sealed one-gold eat-to-win **on smoke / t5 / t8 / t9**;
weekend **datagen** is `--multi-gold` (0–3 golds, openings any).
`weighted_loss` / collation untouched, so t3/t4 do NOT trigger.
...
Multi-gold training datagen (2026-08-25): weekend datagen passes
`--multi-gold` (0–3 golds, openings any, eat-gold is not a win);
weekend smoke / t5 / t8 / t9 stay sealed one-gold eat-to-win.

* **Rerun t1** (seconds — `parse_target` / TARGET line (incl. the
  `Analyst:` same-line self-label); `oracle_verdict` END_GAME
  correct/wrong; `_oracle_meta` validation + `oracle_target_status`
  three-way: ok / mismatch (real object, wrong objective — player
  mistake, trains, verdict punishes) / corrupt (missing or nonexistent
  object — dropped like missing RATING, perception excused);
  `agent_exited` disc contact; `_multi_gold_stop` `exited` arg;
  `DATAGEN_ROOM_FLAG` on `_datagen` and absent from `_smoke_eval`).
  Datagen start heals the updated `core_analyst_045_target_line`
  Preference (perception rounds may write `TARGET: none`; report the
  player's ACTUAL target even when wrong). Exit is disc contact with
  the unit-square boundary, not the agent center leaving `[0,1]`.
* **Optional GPU check** that the factory actually runs (t5 does not
  prove this): `python -m training.generate_game_traces --label
  selftest_multi_gold --multi-gold --games 2 --max-moves 5 --parallel 2
