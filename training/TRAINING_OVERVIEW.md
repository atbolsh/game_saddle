...
| 5+ | Possible: analyst training under a verified metric ([FUTURE_GOALS.md](../FUTURE_GOALS.md) goal 14 — on ice), prompt internalization + instinct-move default (staged plan in [TRAINING_TRACE_EXTRAS.md](TRAINING_TRACE_EXTRAS.md)), image-decoder graft (see [IMAGE_DECODER_GRAFT.md](../IMAGE_DECODER_GRAFT.md)) | discussion |
...
(12-wide T~8k is 57.5 GiB; 12×12288 leftover-pad is 93.3 GiB, so
`T>=8700` isolates to GPU B≤3; 16 concurrent KV caches plus the NAMS
MiniLM embedder on the same GPU died with
