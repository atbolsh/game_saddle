...
Stage map (rationale in the Intermission plan):
  ...
  * t1-pure     parse_rating, parse_target (TARGET: gold|openings, i /
                ...
                correct/wrong + action-balance 1.0; unified prompt
                composition (C' each-turn order, current-screen tip,
                aim-tolerance ray-hit); core-tip numbered categories +
                labeled dump shape; batch-cap-aware step estimate;
                cosine floor clamp
...
def t1_pure() -> str:
    ...
    assert "core_player_035_current_screen" in player_map
    assert "does not mean you are facing it now" in modes._BLOCK_CURRENT_SCREEN
    each = modes._BLOCK_EACH_TURN
    assert each.index("2. REASON") < each.index("3. CHECK your target note"), each
    assert "that number times 5 is the turn count" in each
    assert "do not rotate off a hit" in modes._BLOCK_AIM_TOLERANCE
    assert modes._BLOCK_AIM_TOLERANCE in each
    assert modes._BLOCK_AIM_TOLERANCE in modes._BLOCK_AIM_TOLERANCE_REVIEW
