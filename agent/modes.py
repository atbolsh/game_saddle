...
_BLOCK_TARGET_GRADING = (
    ...
    "  - the reply RE-SAVES the target it already has. Test for this "
    "exactly: the notepad shown in your context already has a 'target' "
    "note; that target is still in the settings and the Board update "
    "did not report it eaten; and the reply's [REMEMBER target: ...] "
    "line maps to that SAME object under your coordinate-mapping rule "
    "(same kind, same gold or opening -- different wording does NOT "
    "make it a different target). The notepad is shown to the player "
    "every turn, so a redundant re-save is pure noise and proof the "
    "player did not read it. When you find one, copy the player's "
    "ENTIRE [REMEMBER target: ...] line, exactly as the player wrote "
    "it, into a WRONG line:\n"
